...
st.set_page_config(layout="wide", page_title="Solar Resource Dashboard")

st.title("🌞 Solar Resource Analysis & Country Comparison Dashboard")

# ---------------- Upload Multiple CSV Files ----------------
st.sidebar.header("📁 Upload Country CSV Files")
uploaded_files = st.sidebar.file_uploader(
    "Upload multiple cleaned datasets", 
    type=["csv"], 
    accept_multiple_files=True
)
...
if uploaded_files:
    ...
    for file in uploaded_files:
        df = pd.read_csv(file)
        country_name = file.name.replace(".csv", "")
        data_dict[country_name] = df

    st.sidebar.success(f"Uploaded {len(data_dict)} files.")

    # ---------------- Metric Selection ----------------
    st.sidebar.header("📈 Select Metric")
    numeric_cols = list(next(iter(data_dict.values())).select_dtypes(include="number").columns)
    metric = st.sidebar.selectbox("Choose a numeric column for comparison", numeric_cols)

    # ---------------- Time-Series Plot ----------------
    st.subheader(f"📉 Time Series Plot for {metric}")
    ...
    # ---------------- Correlation Heatmap ----------------
    st.subheader(f"🔍 Correlation Heatmap for Each Country")
    ...
    # ---------------- Cross-Country Boxplot ----------------
    st.subheader(f"📦 Boxplot — Comparing {metric} Across Countries")
    ...
    for country in plot_df["Country"].unique():
        fig_box.add_trace(go.Box(
            y=plot_df[plot_df["Country"] == country][metric],
            name=country,
            boxmean=True
        ))
